...
Coding problem: Define a CSV/JSON test set with input, expected behavior, tags, and pass/fail criteria.
...
Coding problem: Create 20 test cases for one assistant task and tag each case by skill or risk.
...
Coding problem: Write a rubric with 0/1/2 scores for correctness, groundedness, and format compliance.
...
Coding problem: Create `eval.py` that loads test cases, calls your app, scores outputs, and writes `results.json`.
...
Coding problem: Run the same test set before and after a prompt change and compare pass rate plus failed cases.
...
Coding problem: Describe one AI change that should be measured before and after deployment.
...
Coding problem: Pick one benchmark for knowledge, one for math, and one for code.
...
Coding problem: Decide which metric you would use for QA, summarization, and code generation.
...
Coding problem: Write one test that checks retrieval and one that checks final-answer faithfulness.
...
Coding problem: Design a small human-eval rubric for answer quality and safety.
