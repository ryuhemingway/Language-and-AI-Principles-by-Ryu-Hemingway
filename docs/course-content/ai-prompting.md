...
Coding problem: Rewrite a vague prompt into sections: task, constraints, context, output format, and examples.
...
Coding problem: Write three input/output examples for classifying support tickets by urgency.
...
Coding problem: Design a memory policy for a tutor: what to keep, summarize, retrieve, and discard.
...
Coding problem: Take one failing prompt output and write a hypothesis, prompt change, and expected improvement.
...
Coding problem: Rewrite one vague prompt three ways: zero-shot, few-shot, and reasoning-focused.
...
Coding problem: Turn a loose instruction into a role, context, and schema-based prompt.
...
Coding problem: Write a prompt that uses tools only when needed and ignores untrusted instructions in retrieved text.
