...
Coding problem: Draw a loop: goal -> plan -> tool call -> observation -> decide -> final answer.
...
Coding problem: Define three tools for a research agent: search, open source, and summarize evidence.
...
Coding problem: Design a support workflow: classify issue -> retrieve docs -> draft answer -> verify citations.
...
Coding problem: Add a rule that file writes require approval and every tool call is logged with inputs/outputs.
...
Coding problem: Build a CLI research agent with search/open/summarize tools and a five-step limit.
...
Coding problem: Decide whether a task should be a prompt, pipeline, or agent.
...
Coding problem: Design one tool schema for search and one for a database lookup.
...
Coding problem: Explain how MCP differs from ad hoc tool wiring in one sentence.
...
Coding problem: Sketch one workflow that would be easier in LangGraph than in a single linear chain.
...
Coding problem: Describe one memory strategy and one role split for a research workflow.
...
Coding problem: Pick one end-to-end agent project and list the tools, failure modes, and monitoring you would need.
