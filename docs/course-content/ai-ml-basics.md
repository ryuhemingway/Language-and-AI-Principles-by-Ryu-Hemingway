...
Coding problem: Define a toy spam classifier dataset with input text, label, train split, validation split, and metric.
...
Coding problem: Pick metrics for spam detection, fraud detection, price prediction, and support-ticket routing.
...
Coding problem: Sketch symptoms of overfitting by comparing training loss, validation loss, and test performance.
...
Coding problem: Review a hypothetical churn dataset and identify fields that might leak future information.
...
Coding problem: Connect one math idea to a concrete ML use case such as loss gradients or similarity search.
...
Coding problem: Describe how you would split a small dataset and why the test set stays untouched.
...
Coding problem: Pick one classical algorithm for regression, one for classification, and one for text.
...
Coding problem: Choose an unsupervised method for clustering users, compressing embeddings, and finding outliers.
...
Coding problem: Pick the right metric for spam filtering, churn, and price prediction.
...
Coding problem: Show how you would clean a mixed numeric/categorical dataset before training.
