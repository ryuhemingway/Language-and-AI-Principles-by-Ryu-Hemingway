...
def programming_doc(language: str) -> tuple[str, int]:
    ...
    for lesson in lessons:
        ...
        if lesson.get("alignment_note"):
            lines += [
                f"Course alignment: {lesson['alignment_note']}",
                "",
            ]
...
def ai_doc(module: str) -> tuple[str, int]:
    ...
    for lesson in lessons:
        ...
        lines += [
            ...
            f"Quick check: {quiz}",
            "",
            f"Concept check: {view.get('concept_prompt', '')}",
            "",
        ]
        lines.append("")
...
def main() -> None:
    expected_programming = {f"programming-{language}.md" for language in learn.LEARN_LANGUAGES}
    for stale in OUT_DIR.glob("programming-*.md"):
        if stale.name not in expected_programming:
            stale.unlink()
